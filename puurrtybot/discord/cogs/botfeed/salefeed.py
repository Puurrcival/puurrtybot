...
class SaleFeed(commands.Cog):
    def __init__(self, bot: commands.bot.Bot):
        self.bot = bot

    async def static_loop(self):
        try:
            print('SaleFeed running')
            if not dq.fetch_row(jpgstore.get_sale_last(POLICY_ID)):        
                ...
                for sale in confirmed_sales:        
                    ...
                    tweepy.tweet_sale(content, sale.asset_id)
                    print("sent sale tweet")
        except Exception as e:
            print(f"""SaleFeed error occured: {e.message}""")
...
async def setup(bot: commands.Bot) -> None:
    await bot.add_cog(SaleFeed(bot), guilds = [discord.Object(id = 998148160243384321)])