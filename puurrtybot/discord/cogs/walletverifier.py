...
class WalletVerifier(commands.Cog):
    ...
    async def verify_wallet(self, interaction: discord.Interaction, *, address: str):
        ...
        if amount:
            ...
            for _ in range(20):
                if await verify_transaction(address.address, time_window, str(amount), address_list) and not interaction.is_expired():
                    di.insert_row(address)
                    await update_role_all_by_user(dq.fetch_row(User(address.user_id))) 
                    if not interaction.is_expired(): await message.edit(content = f"""Verification of **{address.address}** successful.""")
                    break
                await asyncio.sleep(time_window)
