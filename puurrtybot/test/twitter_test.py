...
def test_get_id_by_user():
    # exists
    assert TWITTER_ID == twitter.get_id_by_user(TWITTERHANDLE)

    # exists not
    assert not twitter.get_id_by_user("pu33rtyb0tdo")


def test_get_user_by_id():
    # exists
    assert TWITTERHANDLE.strip('@').lower() == twitter.get_user_by_id(TWITTER_ID)
...
def test_get_mentions_by_twitter_id():
    assert type(twitter.get_mentions_by_twitter_id(TWITTER_ID)['meta']) is dict
